...
function doPost(e) {
  ...
  try {
    switch (action) {
      ...
      case 'getComments':
        return getComments(payload.newsId);
      case 'addComment':
        return handleAddComment(payload);
      case 'addBadWord':
        return addRowToSheet('BadWords', payload);
      default:
...
function setup() {
  const sheets = {
    ...
    'Comments': ['NewsID', 'Name', 'Content', 'Date', 'Status'],
    'BadWords': ['Word']
  };
...
function getComments(newsId) {
  ...
  const comments = rows
    ...
    .filter(c => c.NewsID === newsId && c.Status === 'مقبول');
    
  return createResponse(comments);
}

function handleAddComment(payload) {
  const badWords = ['مسيء', 'مضلل', 'سيء', 'كاذب', 'شتم', 'سب', 'قبيح', 'كذب', 'نصاب', 'احتيال', 'حرامي', 'كلب', 'حمار', 'غبي', 'تيس'];
  
  const badWordsSheet = ss.getSheetByName('BadWords');
  if (badWordsSheet) {
      const data = badWordsSheet.getDataRange().getValues();
      if (data.length > 1) {
          for(let i = 1; i < data.length; i++) {
              if (data[i][0]) badWords.push(data[i][0].toString().trim());
          }
      }
  }
  ...
  if (containsBadWord) {
    return createResponse({ success: false, error: 'عذراً، يحتوي تعليقك على كلمات غير لائقة أو مضللة.' });
  }
  
  const sheet = ss.getSheetByName('Comments');
  if (!sheet) {
    ss.insertSheet('Comments').getRange(1, 1, 1, 5).setValues([['NewsID', 'Name', 'Content', 'Date', 'Status']]);
  }
  
  payload.Date = new Date().toLocaleDateString('ar-EG');
  payload.Status = 'مقبول';
  
  return addRowToSheet('Comments', payload);
}
